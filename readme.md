...
This repository covers quote collection, filtering, and judging. You might be looking for the [accompanying repository]() with a website displaying the data. For an explanation of the method, see [here]().
...
1. Fetching from government websites and some inofficial mirrors with `tracker fetch`. This can take quite a while, up to a few days. For the US sources, you'll need to set `GOVINFO_API_KEY` in `.env`.
